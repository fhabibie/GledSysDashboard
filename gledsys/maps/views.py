...
from modules.chart import get_lighting_bar_chart, get_lighting_pie_chart
...
def lighning_temporal_chart(request):
    ...
    if len(query) > 0:
        bar_chart = get_lighting_bar_chart(query)
        bar_chart = "data:image/png;base64, " + bar_chart
        pie_chart = get_lighting_pie_chart(query)
...
def get_lightning_temporal_chart(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == "POST":
        ...
        if len(query) > 0:
            ...
            pie_chart = get_lighting_pie_chart(query)
            pie_chart = "data:image/png;base64, " + pie_chart
            return JsonResponse({"success":True, "data": {"bar": bar_chart, "pie": pie_chart}}, status = 200)
        else:
